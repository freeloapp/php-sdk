...
- **`TaskLabelResource::merge()`** — new `POST /task-labels/merge` endpoint. Merges one or more source labels into a target label server-side: every task carrying a source label ends up carrying the target label instead, and the source labels are detached from those tasks (their definitions are not deleted). The target's name and color come from the existing target label; all UUIDs must be owned by the caller (404 otherwise) and the replacement only touches tasks in projects where the caller is a commander.
